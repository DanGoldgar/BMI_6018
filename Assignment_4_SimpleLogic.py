...
# Initiate array influenza_genome
...
# 1.3 for the last 30 elements, if the index value at that point is divisible by 5, replace the value with "a".
for x in range(len(influenza_genome)-30,len(influenza_genome)):
    if influenza_genome[x] %5 == 0:
        influenza_genome[x]="a"
one_three = influenza_genome[len(influenza_genome)-30:len(influenza_genome)]
print("Problem 1.3")
print(one_three)
...
def three_one(input_array):
    sequence = input_array
    output = []
    for x in range(0,10):
        output.append(sequence[x*25])
    return output
...
def three_two(input_array, y = 10):
    sequence = input_array
    output = []
    # Check to ensure iteration does not exceed array length
    if (y*20)<len(sequence):
        for x in range(0,20):
            output.append(sequence[x*y])
    return output
...
def three_three(input_array):
    sequence = three_two(input_array)
...
def four_one(input_array):
    sequence = input_array
    for x in range(200, 301):
        if sequence[x] % 3 == 0 and sequence[x] % 5 == 0:
            sequence[x] = 10
    for x in range(234, 238):
        if x == 236:
            i = 0
